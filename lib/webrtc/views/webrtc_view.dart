...
class _WebRTCViewState extends State<WebRTCView> with TickerProviderStateMixin {
  ...
  void _startCall() async {
    ...
    try {
      print('🤝 Joining call (will connect to existing peers if any)');
      // Always use joinCall - the backend will handle whether there are existing peers or not
      if (widget.isVideoCall) {
        await provider.joinCall(videoEnabled: true, audioEnabled: true);
      } else {
        await provider.joinCall(videoEnabled: false, audioEnabled: true);
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) async {
        ...
        // Check if we're in a connected call
        final provider = context.read<WebRTCProvider>();
        if (provider.callState == CallState.connected) {
          // Minimize the call instead of closing
          CallOverlayManager.showMinimizedCall(
            context: context,
      ...
      child: Scaffold(
        backgroundColor: const Color(0xFF0B141B), //  dark background
  ...
  Widget _buildLocalVideoPreview(WebRTCProvider provider) {
    ...
    return Positioned(
      ...
      child: GestureDetector(
        onTap: () {
          HapticFeedback.lightImpact();
          // Show message that camera switching is coming soon
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Camera switching feature coming soon!'),
